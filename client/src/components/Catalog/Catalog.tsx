import {Container, Divider, Grid, IconButton, InputBase, Paper, Typography} from "@mui/material";
...
const Catalog = () => {
    ...
    const filter = (product: Product) => {
        return product.instrumentName.toLocaleLowerCase().includes(searchText.toLocaleLowerCase()) ||
            product.category.toLocaleLowerCase().includes(searchText.toLocaleLowerCase())
    }
    ...
    <Container sx={{display: "flex", flexDirection: "column", alignItems: "center"}}>
        ...
        <Grid container spacing={4}>
            {allProducts.filter(filter).map((product: Product) => (
